	...
	registerSuite({
		...
		"Textbox case upper": function () {
			...
			return loadFile(remote, "./Textbox.html")
				...
				.then(function () {
					...
					if (/safari|iOS/.test(remote.environmentType.browserName)) {
					//	return this.skip();
		...
		"Textbox selectInputText": function () {
			...
			return loadFile(remote, "./Textbox.html")
				...
				.end()
				.then(function () {
					if (/firefox|iOS/.test(remote.environmentType.browserName)) {
						// Webdriver issues for now with FF and problem on iOS
					//	return this.skip();
		...
		"Textbox maxLength": function () {
			...
			return loadFile(remote, "./Textbox.html")
				...
				.then(function (maxLength) {
					console.log("maxLength for #maxLengthtb9 > input = "+maxLength);
				...
				.getProperty("maxLength")
				.then(function (maxLength) {
					console.log("maxLength #maxLengthtb9 = "+maxLength);
					assert.equal(maxLength, 8,
			...
			"Textbox minLength": function () {
				this.timeout = intern.config.TEST_TIMEOUT;
				var remote = this.remote;
				return loadFile(remote, "./Textbox.html")
					.then(pollUntil("return document.getElementById('minLengthtb10');", [],
							intern.config.WAIT_TIMEOUT, intern.config.POLL_INTERVAL))
				// this did not help with the minLength property being null on the input
				//	.findByCssSelector("#minLengthtb10")
				//	.then(function (wid) {
				//		wid.minLength = 8;
				//		console.log("set wid.minLength=8 = "+wid.minLength);
				//	})
					.end()
					.findByCssSelector("#minLengthtb10 > input")
					.getProperty("minLength")
					.then(function (minLength) {
						console.log("minLength property for #minLengthtb10 > input = "+minLength);
					//TODO: why is minLength property null on the input field?
					//	assert.equal(minLength, 8,
					//		"Unexpected minLength property for #minLengthtb10 > input");
					})
					.getAttribute("minlength")
					.then(function (minlength) {
						console.log("minlength attribute for #minLengthtb10 > input = "+minlength);
						assert.equal(minlength, 8,
							"Unexpected minlength attribute for #minLengthtb10 > input");
					})
					.end()
					.findByCssSelector("#minLengthtb10")
					.getProperty("minLength")
					.then(function (minLength) {
						console.log("minLength #minLengthtb10 = "+minLength);
						assert.equal(minLength, 8,
							"Unexpected minLength for #minLengthtb10");
					})
					.end()
					.findByCssSelector("#minLengthtb10 > input").click()
					.then(function () {
						checkValidity(remote, "minLengthtb10", "0123", false, false, false, false);
						return checkTextbox(remote, "minLengthtb10", "0123", false, false);
					})
					.end()
					.findByCssSelector("#minLengthtb10 > input").click()
					.clearValue()
					.sleep(100)
					.pressKeys("0123456789")
					.end()
					.then(function () {
						checkValidity(remote, "minLengthtb10", "0123456789", true, false, false, false);
						return checkTextbox(remote, "minLengthtb10", "0123456789", false, false);
					})
					.end()
